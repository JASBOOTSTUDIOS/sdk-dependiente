...
static int validate_user_defined_name_tok(Parser *p, const Token *tok) {
    if (!tok) return 1;
    if (tok->type != TOK_IDENTIFIER && tok->type != TOK_KEYWORD) return 1;
...
static ASTNode *parse_function(Parser *p, int is_exported, int is_async, Visibility visibility) {
...
static ASTNode *parse_struct_body(Parser *p, int is_clase) {
    ...
    Visibility struct_visibility = VISIBILITY_PUBLIC;
    if (match_word(p, "privado")) {
        struct_visibility = VISIBILITY_PRIVATE;
    }
    ...
    size_t nf = 0, cap = 0, nm = 0, mcap = 0;

    while (peek(p, 0) && peek(p, 0)->value.str && !struct_kw_is_closer(peek(p, 0)->value.str, is_clase)) {
        Visibility current_member_visibility = VISIBILITY_PUBLIC;
        if (match_word(p, "privado")) {
            current_member_visibility = VISIBILITY_PRIVATE;
        }
        ...
        if (tblk && tblk->type == TOK_KEYWORD && tblk->value.str) {
            ...
            if (strcmp(kw, "funcion") == 0 ||
                (is_clase && strcmp(kw, "enviar") == 0 && peek(p, 1) &&
                 peek(p, 1)->type == TOK_KEYWORD && peek(p, 1)->value.str &&
                 strcmp(peek(p, 1)->value.str, "funcion") == 0)) {
                if (!is_clase) {
                    if (p->source_path && p->source_path[0])
                        set_error_at(p, tblk->line, tblk->column,
                            "Archivo %s, linea %d, columna %d: dentro de registro '%s' no se pueden declarar metodos; use un campo `funcion nombre` para puntero a funcion.",
                            p->source_path, tblk->line, tblk->column, name ? name : "?");
                    else
                        set_error_at(p, tblk->line, tblk->column,
                            "linea %d, columna %d: dentro de registro no se permiten metodos; use un campo `funcion nombre`.",
                            tblk->line, tblk->column);
                    goto parse_struct_body_fail;
                }
                int is_exported = 0;
                if (strcmp(kw, "enviar") == 0) {
                    is_exported = 1;
                    advance(p);
                }
                ASTNode *method_node = parse_function(p, is_exported, 0, current_member_visibility);
                if (!method_node) goto parse_struct_body_fail;
                FunctionNode *mf = (FunctionNode *)method_node;
                ASTNode **new_params = calloc(mf->n_params + 1, sizeof(ASTNode *));
                VarDeclNode *self_param = calloc(1, sizeof(VarDeclNode));
                if (!new_params || !self_param) {
                    free(new_params);
                    free(self_param);
                    ast_free(method_node);
                    goto parse_struct_body_fail;
                }
                self_param->base.type = NODE_VAR_DECL;
                self_param->base.line = mf->base.line;
                self_param->base.col = mf->base.col;
                self_param->type_name = name ? strdup(name) : NULL;
                self_param->name = strdup("este");
                new_params[0] = (ASTNode *)self_param;
                for (size_t i = 0; i < mf->n_params; i++)
                    new_params[i + 1] = mf->params[i];
                free(mf->params);
                mf->params = new_params;
                mf->n_params++;
                if (nm >= mcap) {
                    mcap = mcap ? mcap * 2 : 4;
                    ASTNode **next_methods = realloc(methods, mcap * sizeof(ASTNode *));
                    if (!next_methods) {
                        ast_free(method_node);
                        goto parse_struct_body_fail;
                    }
            ...
            if (strcmp(kw, "principal") == 0 || strcmp(kw, "asincrono") == 0 ||
                ...
                strcmp(kw, "extiende") == 0 || strcmp(kw, "privado") == 0 ||
        ...
        if (fld->type != TOK_IDENTIFIER) {
            if (p->source_path && p->source_path[0])
                set_error_at(p, fld->line, fld->column,
                          "Archivo %s, linea %d, columna %d: se esperaba un nombre de campo, no `%s`.",
                          p->source_path, fld->line, fld->column, fld->type == TOK_NUMBER ? "un numero" : "este token");
            else
                set_error_at(p, fld->line, fld->column, "linea %d, columna %d: se esperaba un nombre de campo.", fld->line, fld->column);
            goto parse_struct_body_fail;
        }
        ...
        if (nf >= cap) {
            ...
            Visibility *pfv = realloc(fv, cap * sizeof(Visibility));
            if (!pft || !pfn || !pfv) break;
            ft = pft;
            fn = pfn;
            fv = pfv;
        }
        ...
        fv[nf] = current_member_visibility;
        nf++;
    }
    ...
    sn->visibility = struct_visibility;
    sn->field_types = ft;
    sn->field_names = fn;
...
static void export_symbol_by_name(NodeVec *funcs, NodeVec *globals, const char *name) {
    if (!name) return;
    for (size_t i = 0; i < funcs->n; i++) {
        FunctionNode *fn = (FunctionNode *)funcs->arr[i];
        ...
        if (t->type == TOK_KEYWORD && t->value.str) {
            if (strcmp(t->value.str, "asincrono") == 0) {
                ...
                if (strcmp(t2->value.str, "enviar") == 0) {
                    ...
                    if (t3 && t3->type == TOK_KEYWORD && t3->value.str && strcmp(t3->value.str, "funcion") == 0) {
                        ASTNode *fn = parse_function(p, 1, 1, VISIBILITY_PUBLIC);
                        if (!fn) {
                            if (p->last_error) {
                                parser_accumulate_error(p, p->last_error);
                                free(p->last_error);
                ...
                if (strcmp(t2->value.str, "funcion") == 0) {
                    ASTNode *fn = parse_function(p, 0, 1, VISIBILITY_PUBLIC);
                    if (!fn) {
                        if (p->last_error) {
                            parser_accumulate_error(p, p->last_error);
                            free(p->last_error);
            ...
            if (strcmp(t->value.str, "enviar") == 0) {
                ...
                if (nx && nx->type == TOK_OPERATOR && nx->value.str && strcmp(nx->value.str, "{") == 0) {
                    ...
                    while (1) {
                        const Token *id = peek(p, 0);
                        if (!id || id->type == TOK_EOF) break;
                        if (id->type == TOK_OPERATOR && id->value.str && strcmp(id->value.str, "}") == 0) {
                            advance(p);
                            break;
                        }
                        if (id->type == TOK_IDENTIFIER) {
                            export_symbol_by_name(&funcs, &globals, id->value.str);
                            advance(p);
                        } else {
                            set_error_here(p, id, "Se esperaba un nombre de funcion, variable o clase en la lista de `enviar { ... }`.");
                            parser_accumulate_error(p, p->last_error);
                            free(p->last_error);
                            p->last_error = NULL;
                            parser_synchronize(p);
                        ...
                        set_error_here(p, id, "Tras un nombre en `enviar { ... }` se esperaba `,` o `}`.");
                        parser_accumulate_error(p, p->last_error);
                        free(p->last_error);
                        p->last_error = NULL;
                        parser_synchronize(p);
                 ...
                 if (nx && nx->type == TOK_IDENTIFIER) {
                     export_symbol_by_name(&funcs, &globals, nx->value.str);
                     advance(p);
                     continue;
                 }
                 if (nx && nx->type == TOK_KEYWORD && nx->value.str && strcmp(nx->value.str, "funcion") == 0) {
                    ASTNode *fn = parse_function(p, 1, 0, VISIBILITY_PUBLIC);
                    if (!fn) {
                        if (p->last_error) {
                            parser_accumulate_error(p, p->last_error);
                            free(p->last_error);
                ...
                if (s->type == NODE_STRUCT_DEF) {
                    // Exportar clase/registro
                    ((StructDefNode *)s)->is_exported = 1;
                    node_vec_push(&globals, s);
                    continue;
                }
            ...
            if (strcmp(t->value.str, "funcion") == 0) {
                ASTNode *fn = parse_function(p, 0, 0, VISIBILITY_PUBLIC);
                if (!fn) {
                    if (p->last_error) {
                        parser_accumulate_error(p, p->last_error);
                        free(p->last_error);
