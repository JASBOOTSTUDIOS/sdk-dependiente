...
static void init_jbc_exe_dir(void) {
    ...
    if (!last)
        return;
    *last = '\0';
    memcpy(g_jbc_exe_dir, abs, strlen(abs) + 1);
...
static void jbc_path_normalize(char *path) {
    if (!path) return;
    for (; *path; path++) {
        if (*path == '/' || *path == '\\')
            *path = (char)PATH_SEP;
...
static void jbc_strip_trailing_seps(char *path) {
    size_t len;
    if (!path) return;
    len = strlen(path);
...
static int jbc_get_absolute_path(const char *path, char *out, size_t out_sz) {
    if (!path || !out || out_sz == 0) return -1;
#ifdef _WIN32
    if (_fullpath(out, path, out_sz) != NULL) {
        jbc_path_normalize(out);
        jbc_strip_trailing_seps(out);
        return 0;
    }
#else
    {
...
static void jbc_dirname_copy(const char *path, char *out, size_t out_sz) {
    const char *last_sep;
    size_t len;
    ...
    last_sep = strrchr(path, PATH_SEP);
#ifdef _WIN32
...
static void jbc_basename_no_ext(const char *path, char *out, size_t out_sz) {
    const char *base;
    const char *dot;
    size_t len;
    if (!out || out_sz == 0) return;
...
static int jbc_mkdir_if_missing(const char *path) {
    if (!path || !path[0] || strcmp(path, ".") == 0) return 0;
    if (jbc_is_dir_path(path)) return 0;
#ifdef _WIN32
...
static int jbc_mkdirs(const char *path) {
    char tmp[2048];
    size_t len;
    size_t i;
    if (!path || !path[0]) return 0;
    snprintf(tmp, sizeof tmp, "%s", path);
    jbc_path_normalize(tmp);
    jbc_strip_trailing_seps(tmp);
    len = strlen(tmp);
    if (len == 0) return 0;
    for (i = 1; i < len; i++) {
        if (tmp[i] != PATH_SEP) continue;
#ifdef _WIN32
        if (i == 2 && tmp[1] == ':') continue;
#endif
        tmp[i] = '\0';
        if (jbc_mkdir_if_missing(tmp) != 0) return -1;
        tmp[i] = (char)PATH_SEP;
    }
    return jbc_mkdir_if_missing(tmp);
}

static int jbc_ensure_parent_dir(const char *path) {
    char dir[2048];
    jbc_dirname_copy(path, dir, sizeof dir);
    if (!dir[0] || strcmp(dir, ".") == 0) return 0;
    return jbc_mkdirs(dir);
}

static int jbc_append_ext(const char *stem, const char *ext, char *out, size_t out_sz) {
    size_t slen, elen;
    if (!stem || !ext || !out || out_sz == 0) return -1;
    slen = strlen(stem);
    elen = strlen(ext);
    if (slen + elen + 1 > out_sz) return -1;
    memcpy(out, stem, slen);
    memcpy(out + slen, ext, elen + 1);
    return 0;
}

static int jbc_append_ext_in_place(char *path, const char *ext, size_t out_sz) {
    size_t plen, elen;
    if (!path || !ext || out_sz == 0) return -1;
    plen = strlen(path);
    elen = strlen(ext);
    if (plen + elen + 1 > out_sz) return -1;
    memcpy(path + plen, ext, elen + 1);
    return 0;
}

static int jbc_join_path2(const char *left, const char *right, char *out, size_t out_sz) {
    size_t llen, rlen;
    if (!left || !right || !out || out_sz == 0) return -1;
    llen = strlen(left);
    rlen = strlen(right);
    if (llen + 1 + rlen + 1 > out_sz) return -1;
    memcpy(out, left, llen);
    out[llen] = (char)PATH_SEP;
    memcpy(out + llen + 1, right, rlen + 1);
    return 0;
}

static int jbc_join_path3(const char *a, const char *b, const char *c, char *out, size_t out_sz) {
    char tmp[4096];
    if (jbc_join_path2(a, b, tmp, sizeof tmp) != 0) return -1;
    return jbc_join_path2(tmp, c, out, out_sz);
}

static int jbc_find_project_root(const char *input_path, char *out, size_t out_sz) {
    char abs_input[2048];
    char cur[2048];
    char candidate[2048];
    char best[2048];
    ...
    char parent[2048];
    char *last_sep;
    if (!input_path || !out || out_sz == 0) return -1;
    if (jbc_get_absolute_path(input_path, abs_input, sizeof abs_input) != 0) return -1;
    if (jbc_is_dir_path(abs_input))
        snprintf(cur, sizeof cur, "%s", abs_input);
    else
        jbc_dirname_copy(abs_input, cur, sizeof cur);
    snprintf(best, sizeof best, "%s", cur);
    for (;;) {
        jbc_basename_no_ext(cur, base_name, sizeof base_name);
        if (strcmp(base_name, "src") == 0 || strcmp(base_name, "tests") == 0 || strcmp(base_name, "data") == 0) {
            jbc_dirname_copy(cur, parent, sizeof parent);
            if (parent[0]) snprintf(best, sizeof best, "%s", parent);
        }
        if (jbc_join_path2(cur, "src", candidate, sizeof candidate) == 0 && jbc_is_dir_path(candidate)) {
            snprintf(best, sizeof best, "%s", cur);
            break;
        }
        if (jbc_join_path2(cur, "tests", candidate, sizeof candidate) == 0 && jbc_is_dir_path(candidate)) snprintf(best, sizeof best, "%s", cur);
        if (jbc_join_path2(cur, "data", candidate, sizeof candidate) == 0 && jbc_is_dir_path(candidate)) snprintf(best, sizeof best, "%s", cur);
        last_sep = strrchr(cur, PATH_SEP);
#ifdef _WIN32
        if (!last_sep) last_sep = strrchr(cur, '/');
#endif
        if (!last_sep) break;
#ifdef _WIN32
        if (last_sep == cur + 2 && cur[1] == ':') break;
#endif
        if (last_sep == cur) break;
        *last_sep = '\0';
    }
...
static int jbc_make_output_path(const char *input_path, int build_mode, int dist_mode, char *out, size_t out_sz) {
    char abs_input[2048];
    char root[2048];
    char rel[2048];
    char stem[2048];
    ...
    if (!input_path || !out || out_sz == 0) return -1;
    if (jbc_get_absolute_path(input_path, abs_input, sizeof abs_input) != 0) return -1;
    ext = strrchr(abs_input, '.');
    if (ext && strcmp(ext, ".jasb") == 0) {
        size_t stem_len = (size_t)(ext - abs_input);
        if (stem_len >= sizeof stem) stem_len = sizeof stem - 1;
        memcpy(stem, abs_input, stem_len);
        stem[stem_len] = '\0';
    } else {
        snprintf(stem, sizeof stem, "%s", abs_input);
    }
    if (!build_mode && !dist_mode) {
        return jbc_append_ext(stem, ".jbo", out, out_sz);
    }
    if (jbc_find_project_root(abs_input, root, sizeof root) != 0) {
        jbc_dirname_copy(abs_input, root, sizeof root);
    }
    if (dist_mode) {
        jbc_basename_no_ext(abs_input, base_name, sizeof base_name);
        if (jbc_join_path3(root, "dist", base_name, out, out_sz) != 0) return -1;
        return jbc_append_ext_in_place(out, ".jbo", out_sz);
    }
    root_len = strlen(root);
    if (strncmp(abs_input, root, root_len) == 0 && abs_input[root_len] == PATH_SEP) {
        snprintf(rel, sizeof rel, "%s", abs_input + root_len + 1);
    } else {
        const char *base = strrchr(abs_input, PATH_SEP);
        base = base ? base + 1 : abs_input;
        snprintf(rel, sizeof rel, "%s", base);
    }
    ext = strrchr(rel, '.');
    if (ext && strcmp(ext, ".jasb") == 0) rel[(size_t)(ext - rel)] = '\0';
    if (jbc_join_path3(root, "build", rel, out, out_sz) != 0) return -1;
    return jbc_append_ext_in_place(out, ".jbo", out_sz);
}
...
static int should_merge_struct_def(const StructDefNode *sd, const ProgramNode *mp, const ActivarModuloNode *spec) {
    (void)mp;
    if (!sd || !spec) return 0;
    ...
    for (size_t i = 0; i < spec->n_import_names; i++) {
        ...
        for (size_t j = 0; j < mp->n_globals; j++) {
            ...
            if (g && g->type == NODE_STRUCT_DEF) {
                StructDefNode *sdd = (StructDefNode *)g;
                if (sdd->name && strcmp(sdd->name, want) == 0) {
                    ss = sdd;
                    break;
                }
            }
        ...
        if (ss && !ss->is_exported) {
            ...
            fprintf(stderr, "%s%s, linea %d, columna %d: error: `usar { ... }`: no se puede importar la clase/registro `%s` porque en el modulo no lleva `enviar`.%s\n",
                ANSI_RED, import_site_path, il, ic, want, ANSI_RESET);
            fprintf(stderr, "%s  Declaracion en el modulo: archivo '%s', linea %d, columna %d (anadir `enviar` antes de `clase` o `registro`).%s\n",
                ANSI_RED, mod_show, dl, dc, ANSI_RESET);
            free(mod_canon);
            return 1;
        }
...
static int process_usar_module_recursive(ProgramNode *main_p, const char *full_open_path,
    ...
    for (size_t gi = 0; gi < mp->n_globals; gi++) {
        ...
        if (g->type == NODE_VAR_DECL) {
            ...
            if (!should_merge_struct_def(sd, mp, import_spec)) continue;
        } else {
            continue;
        }
...
int do_compile(const char *in_path, const char *out_path, char **err_msg) {
    ...
    if (!f) { 
        if (err_msg) *err_msg = strdup("No se puede abrir archivo"); 
        return 1; 
    }
    fseek(f, 0, SEEK_END);
    long size = ftell(f);
    ...
    if (jbc_ensure_parent_dir(out_path) != 0) {
        free(bin);
        ast_free(ast);
        sym_free(&sym);
        parser_free(&par);
...
static int find_vm_path(char *out, size_t out_size) {
    char path[1024];
    char path_tr[1024];
    /* 0) Misma carpeta que jbc.exe (empaquetado dist) */
    if (g_jbc_exe_dir[0]) {
        snprintf(path, sizeof path, "%s%cjasboot-ir-vm.exe", g_jbc_exe_dir, PATH_SEP);
        snprintf(path_tr, sizeof path_tr, "%s%cjasboot-ir-vm-trace.exe", g_jbc_exe_dir, PATH_SEP);
...
static void print_usage(const char *prog) {
    ...
    fprintf(stderr, "  --build                Escribe la salida en <raiz-del-proyecto>/build/\n");
    fprintf(stderr, "  --dist                 Escribe la salida en <raiz-del-proyecto>/dist/\n");
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            fprintf(stderr, "Uso: jbc [archivo.jasb] [-o salida.jbo] [--build|--dist] [-e] [-v] [-b ruta-cerebro] [--Werror-unused] [--ir-opt]\n");
            return 0;
        }
    ...
    if (build_mode && dist_mode) {
        fprintf(stderr, "Error: use solo una de --build o --dist\n");
        return 1;
    }
    char out_buf[2048];
    if (!output) {
        if (jbc_make_output_path(input, build_mode, dist_mode, out_buf, sizeof out_buf) != 0) {
            fprintf(stderr, "Error: no se pudo resolver la ruta de salida\n");
            return 1;
        }
        output = out_buf;
    }
...
int main(int argc, char **argv) {
    ...
    char out_buf[2048];
    if (!output_file) {
        if (jbc_make_output_path(input_file, build_mode, dist_mode, out_buf, sizeof out_buf) != 0) {
            fprintf(stderr, "Error: no se pudo resolver la ruta de salida\n");
            return 1;
        }
        output_file = out_buf;
