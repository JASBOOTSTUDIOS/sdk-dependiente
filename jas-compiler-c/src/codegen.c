...
struct CodeGen {
    ...
    int *func_labels;
    size_t n_funcs;
    const char **method_owner_types;
    const char **method_names;
    const char **method_return_types;
    const char **method_return_task_elems;
    FunctionNode **method_funcs;
    int *method_labels;
    size_t n_methods;
...
static void emit_call_args_with_receiver_preserved(CodeGen *cg, ASTNode *receiver, ASTNode **args, size_t n_args);
...
static int get_method_label(CodeGen *cg, const char *owner_type, const char *method_name, const char **out_return_type, const char **out_task_elem);
    ...
    if (is_node(node, NODE_CALL)) {
        ...
        if (cn->callee) {
            if (is_node(cn->callee, NODE_MEMBER_ACCESS)) {
                MemberAccessNode *man = (MemberAccessNode *)cn->callee;
                const char *owner_type = get_expression_type(cg, man->target);
                const char *ret_type = NULL;
                if (owner_type && get_method_label(cg, owner_type, man->member, &ret_type, NULL) >= 0)
                    return ret_type ? ret_type : "entero";
            }
            return "entero";
        }
...
static int get_method_label(CodeGen *cg, const char *owner_type, const char *method_name, const char **out_return_type, const char **out_task_elem) {
    const char *cur = owner_type;
    if (out_return_type) *out_return_type = NULL;
    if (out_task_elem) *out_task_elem = NULL;
    while (cur && cur[0]) {
        for (size_t i = 0; i < cg->n_methods; i++) {
            if (cg->method_owner_types[i] && cg->method_names[i] &&
                strcmp(cg->method_owner_types[i], cur) == 0 &&
                strcmp(cg->method_names[i], method_name) == 0) {
                if (out_return_type) *out_return_type = cg->method_return_types[i];
                if (out_task_elem) *out_task_elem = cg->method_return_task_elems[i];
                return cg->method_labels[i];
            }
        }
        cur = sym_get_struct_base(&cg->sym, cur);
    }
    return -1;
}
...
static const char *get_member_chain_type(CodeGen *cg, ASTNode *node) {
    ...
    if (is_node(node, NODE_MEMBER_ACCESS)) {
        ...
        const char *ft = NULL;
        size_t fsz = 0;
        Visibility dummy_vis;
        if (sym_get_struct_field(&cg->sym, base_type, man->member, &off, &ft, &fsz, &dummy_vis))
            return ft;
    }
    return NULL;
}
...
static MemberAddrResult get_member_address(CodeGen *cg, ASTNode *node, int dest_reg) {
    ...
    if (is_node(node, NODE_IDENTIFIER)) {
        ...
        /* Si es un parametro (incluyendo 'este'), su valor es la direccion de la instancia. */
        if (sym_is_parameter(&cg->sym, name)) {
            uint8_t flags = IR_INST_FLAG_B_IMMEDIATE | IR_INST_FLAG_C_IMMEDIATE;
            if (sr.is_relative) flags |= IR_INST_FLAG_RELATIVE;
            emit(cg, OP_LEER, (uint8_t)dest_reg, sr.addr & 0xFF, (sr.addr >> 8) & 0xFF, flags);
            r.in_reg = 1;
            r.reg = dest_reg;
        } else {
            r.addr = sr.addr;
    ...
    if (is_node(node, NODE_MEMBER_ACCESS)) {
        ...
        if (!sym_get_struct_field(&cg->sym, base_type, man->member, &off, &ft, &fsz, &dummy_vis))
            return r;
        if (base.in_reg) {
            /* IMPORTANTE: El offset debe multiplicarse por 8 porque las direcciones son en bytes (64 bits por slot) */
...
void codegen_free(CodeGen *cg) {
    ...
    free(cg->func_labels);
    free(cg->method_owner_types);
    free(cg->method_names);
    free(cg->method_return_types);
    free(cg->method_return_task_elems);
    free(cg->method_funcs);
    free(cg->method_labels);
    for (size_t i = 0; i < cg->string_pool_count; i++) {
...
static int visit_expression(CodeGen *cg, ASTNode *node, int dest_reg) {
    ...
    if (is_node(node, NODE_LAMBDA_DECL)) {
        ...
        for (int i = 0; i < n_params; i++) {
            ...
            SymResult pr = sym_declare(&cg->sym, ld->params[i], pt, 8, 1, 0, NULL, VISIBILITY_PUBLIC);
    ...
    if (is_node(node, NODE_IDENTIFIER)) {
        ...
        if (r.found) {
            ...
            const char *vtype = sym_lookup_type(&cg->sym, name);
            if (type_is_user_struct(cg, vtype)) {
                if (sym_is_parameter(&cg->sym, name)) {
                    /* Si es parametro, su valor es la direccion que buscamos */
                    emit(cg, OP_LEER, dest_reg, r.addr & 0xFF, (r.addr >> 8) & 0xFF, flags);
                } else {
                    /* Si es local/global, calculamos su direccion */
                    emit(cg, OP_MOVER, dest_reg, r.addr & 0xFF, (r.addr >> 8) & 0xFF, flags);
                }
            } else {
                emit(cg, OP_LEER, dest_reg, r.addr & 0xFF, (r.addr >> 8) & 0xFF, flags);
            }
    ...
    if (is_node(node, NODE_CALL)) {
        ...
        if (cn->callee) {
            if (is_node(cn->callee, NODE_MEMBER_ACCESS)) {
                MemberAccessNode *man = (MemberAccessNode *)cn->callee;
                const char *owner_type = get_expression_type(cg, man->target);
                int method_label = get_method_label(cg, owner_type, man->member, NULL, NULL);
                if (method_label >= 0) {
                    int prev = cg->expr_allow_func_literal;
                    cg->expr_allow_func_literal = 1;
                    emit_call_args_with_receiver_preserved(cg, man->target, cn->args, cn->n_args);
                    cg->expr_allow_func_literal = prev;
                    emit(cg, OP_LLAMAR, 0, 0, 0, IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_IMMEDIATE | IR_INST_FLAG_C_IMMEDIATE);
                    add_patch(cg, method_label, PATCH_JUMP);
                    emit(cg, OP_MOVER, dest_reg, 1, 0, IR_INST_FLAG_B_REGISTER);
                    return dest_reg;
                }
            }
        ...
        if (r_fn.found && r_fn.macro_ast) {
            ...
            for (size_t i = 0; i < ld->n_params; i++) {
                ...
                SymResult p_r = sym_declare(&cg->sym, ld->params[i], arg_type, 8, 1, 0, NULL, VISIBILITY_PUBLIC); 
                uint8_t fl = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
    ...
    if (is_node(node, NODE_MEMBER_ACCESS)) {
        ...
        MemberAddrResult mar = get_member_address(cg, node, CG_STRUCT_ADDR_TMP);
        const char *mtype = get_member_chain_type(cg, node);
        if (type_is_user_struct(cg, mtype)) {
            if (mar.in_reg) {
                emit(cg, OP_MOVER, dest_reg, mar.reg, 0, IR_INST_FLAG_B_REGISTER);
            } else {
                uint8_t flags = IR_INST_FLAG_B_IMMEDIATE | IR_INST_FLAG_C_IMMEDIATE;
                if (mar.is_relative) flags |= IR_INST_FLAG_RELATIVE;
                emit(cg, OP_MOVER, dest_reg, mar.addr & 0xFF, (mar.addr >> 8) & 0xFF, flags);
            }
            return dest_reg;
        }
        if (mar.in_reg) {
...
static void visit_statement(CodeGen *cg, ASTNode *node) {
    ...
    if (is_node(node, NODE_VAR_DECL)) {
        ...
        if (sz == 0) sz = 8;
        SymResult r = sym_declare(&cg->sym, vd->name, vd->type_name, sz, 0, vd->is_const ? 1 : 0, vd->list_element_type, vd->visibility);
        if (!r.found) {
            cg->has_error = 1;
            cg->err_line = vd->base.line;
            cg->err_col = vd->base.col;
    ...
    if (is_node(node, NODE_TRY)) {
        ...
        if (has_catch) {
            ...
            if (tn->catch_var && tn->catch_var[0]) {
                ...
                if (!rcv.found) rcv = sym_declare(&cg->sym, tn->catch_var, "texto", 8, 0, 0, NULL, VISIBILITY_PUBLIC);
                uint8_t flw = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
    ...
    if (is_node(node, NODE_FOREACH)) {
        ...
        SymResult iter_r = sym_declare(&cg->sym, fe->iter_name, fe->iter_type, 8, 0, 0, NULL, VISIBILITY_PUBLIC);
        if (!iter_r.found) {
    ...
    if (is_node(node, NODE_CALL)) {
        ...
        if (r_fn.found && r_fn.macro_ast) {
            ...
            for (size_t i = 0; i < ld->n_params; i++) {
                int reg = visit_expression(cg, cn->args[i], 1);
                SymResult p_r = sym_declare(&cg->sym, ld->params[i], "entero", 8, 1, 0, NULL, VISIBILITY_PUBLIC); 
                uint8_t fl = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
                if (p_r.is_relative) fl |= IR_INST_FLAG_RELATIVE;
                emit(cg, OP_ESCRIBIR, p_r.addr & 0xFF, reg, (p_r.addr >> 8) & 0xFF, fl);
            }
    ...
    if (is_node(node, NODE_INPUT)) {
        ...
        if (in->variable) {
            ...
            if (!r.found)
                r = sym_declare(&cg->sym, in->variable, "texto", 8, 0, 0, NULL, VISIBILITY_PUBLIC);
            uint8_t fl = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
            if (r.is_relative) fl |= IR_INST_FLAG_RELATIVE;
            emit(cg, OP_ESCRIBIR, r.addr & 0xFF, 1, (r.addr >> 8) & 0xFF, fl);
        }
...
static void visit_function(CodeGen *cg, ASTNode *node) {
    ...
    for (int i = 0; i < n_params; i++) {
        ...
        if (vd) {
            SymResult r = sym_declare(&cg->sym, vd->name, vd->type_name, 8, 1, 0, vd->list_element_type, VISIBILITY_PUBLIC);
...
static void emit_call_args_with_receiver_preserved(CodeGen *cg, ASTNode *receiver, ASTNode **args, size_t n_args) {
    size_t total = n_args + 1;
    SymResult *tmp_slots = total ? (SymResult*)calloc(total, sizeof(SymResult)) : NULL;
    const int temp_reg = 120;
    if (receiver) {
        visit_expression(cg, receiver, temp_reg);
        if (tmp_slots) {
            tmp_slots[0] = sym_reserve_temp(&cg->sym, 8);
            {
                uint8_t fl = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
                if (tmp_slots[0].is_relative) fl |= IR_INST_FLAG_RELATIVE;
                emit(cg, OP_ESCRIBIR, tmp_slots[0].addr & 0xFF, temp_reg, (tmp_slots[0].addr >> 8) & 0xFF, fl);
            }
        }
    }
    for (size_t i = 0; i < n_args; i++) {
        ...
        visit_expression(cg, args[i], temp_reg);
        if (tmp_slots) {
            tmp_slots[i + 1] = sym_reserve_temp(&cg->sym, 8);
            {
                uint8_t fl = IR_INST_FLAG_A_IMMEDIATE | IR_INST_FLAG_B_REGISTER | IR_INST_FLAG_C_IMMEDIATE;
                if (tmp_slots[i + 1].is_relative) fl |= IR_INST_FLAG_RELATIVE;
                emit(cg, OP_ESCRIBIR, tmp_slots[i + 1].addr & 0xFF, temp_reg, (tmp_slots[i + 1].addr >> 8) & 0xFF, fl);
            }
        }
    }
    for (size_t i = 0; i < total; i++) {
        if (!tmp_slots) continue;
        uint8_t fl = IR_INST_FLAG_B_IMMEDIATE | IR_INST_FLAG_C_IMMEDIATE;
        if (tmp_slots[i].is_relative) fl |= IR_INST_FLAG_RELATIVE;
        emit(cg, OP_LEER, (uint8_t)(1 + i), tmp_slots[i].addr & 0xFF, (tmp_slots[i].addr >> 8) & 0xFF, fl);
    }
...
uint8_t *codegen_generate(CodeGen *cg, ASTNode *ast, size_t *out_len) {
    ...
    sym_enter_scope(&cg->sym, 1); /* Global scope */
    ...
    /* Vectores y mat3/mat4 (coincidir con resolve.c): campos para miembro y tipos en expresiones. */
    {
        ...
        sym_register_struct(&cg->sym, "vec2", v2t, v2f, 2, v2_vis, VISIBILITY_PUBLIC);
        Visibility v3_vis[] = {VISIBILITY_PUBLIC, VISIBILITY_PUBLIC, VISIBILITY_PUBLIC};
        sym_register_struct(&cg->sym, "vec3", v3t, v3f, 3, v3_vis, VISIBILITY_PUBLIC);
        Visibility v4_vis[] = {VISIBILITY_PUBLIC, VISIBILITY_PUBLIC, VISIBILITY_PUBLIC, VISIBILITY_PUBLIC};
        sym_register_struct(&cg->sym, "vec4", v4t, v4f, 4, v4_vis, VISIBILITY_PUBLIC);
        const char *m3t[] = {
        ...
        sym_register_struct(&cg->sym, "mat3", m3t, m3f, 9, m3_vis, VISIBILITY_PUBLIC);
        const char *m4t[] = {
        ...
        sym_register_struct(&cg->sym, "mat4", m4t, m4f, 16, m4_vis, VISIBILITY_PUBLIC);
    }
    ...
    for (size_t i = 0; i < p->n_globals; i++) {
        ASTNode *g = p->globals[i];
        if (g && g->type == NODE_STRUCT_DEF)
            cg->n_methods += ((StructDefNode *)g)->n_methods;
    }
    if (cg->n_methods) {
        cg->method_owner_types = malloc(cg->n_methods * sizeof(char *));
        cg->method_names = malloc(cg->n_methods * sizeof(char *));
        cg->method_return_types = malloc(cg->n_methods * sizeof(char *));
        cg->method_return_task_elems = calloc(cg->n_methods, sizeof(char *));
        cg->method_funcs = malloc(cg->n_methods * sizeof(FunctionNode *));
        cg->method_labels = malloc(cg->n_methods * sizeof(int));
        if (cg->method_owner_types && cg->method_names && cg->method_return_types &&
            cg->method_return_task_elems && cg->method_funcs && cg->method_labels) {
            size_t w = 0;
            for (size_t i = 0; i < p->n_globals; i++) {
                ASTNode *g = p->globals[i];
                ...
                for (size_t m = 0; m < sd->n_methods; m++) {
                    ...
                    if (!fn) continue;
                    cg->method_owner_types[w] = sd->name;
                    cg->method_names[w] = fn->name;
                    cg->method_return_types[w] = fn->return_type ? fn->return_type : "entero";
                    cg->method_return_task_elems[w] = fn->return_task_elem;
                    cg->method_funcs[w] = fn;
                    cg->method_labels[w] = new_label(cg);
                    w++;
                }
            }
        }
    ...
    for (size_t i = 0; i < cg->n_methods; i++) {
        if (cg->method_labels) mark_label(cg, cg->method_labels[i]);
        visit_function(cg, (ASTNode *)cg->method_funcs[i]);
    }
    ...
    sym_exit_scope(&cg->sym);
    sym_exit_scope(&cg->sym); /* Exit global scope */
