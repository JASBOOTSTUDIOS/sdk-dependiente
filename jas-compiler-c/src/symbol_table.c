...
void sym_init(SymbolTable *st) {
    st->scope_depth = 0;
    st->next_global_offset = 0;
    st->next_local_offset = 0;
    st->is_global = 1;
    st->current_class_name = NULL;
    st->structs_cap = 32;
    st->structs = calloc(st->structs_cap, sizeof(StructInfo));
    if (!st->structs) {
        fprintf(stderr, "Error: no se pudo asignar memoria para la tabla de simbolos de structs.\n");
        exit(1);
    }
}
...
/* 3.2/3.3/3.4 */
SymResult sym_declare(SymbolTable *st, const char *name, const char *type_name, size_t size, int is_param, int is_const, const char *lista_elem_type, Visibility visibility) {
    SymResult r = {0, 0, NULL, 0, 0, NULL, NULL};
...
SymResult sym_declare_macro(SymbolTable *st, const char *name, void *macro_ast) {
    SymResult r = sym_declare(st, name, "macro", 0, 0, 1, NULL, VISIBILITY_PUBLIC);
    if (r.found) {
...
SymResult sym_reserve_temp(SymbolTable *st, size_t size) {
    SymResult r = {0, 1, NULL, 1, 0, NULL, NULL};
    if (size == 0) size = DEFAULT_SIZE;
...
SymResult sym_get_or_create(SymbolTable *st, const char *name, const char *type_name) {
    ...
    if (r.found)
        return r;
    return sym_declare(st, name, type_name, DEFAULT_SIZE, 0, 0, NULL, VISIBILITY_PUBLIC);
}
...
void sym_register_struct(SymbolTable *st, const char *name, const char **field_types, const char **field_names, size_t n_fields, Visibility *field_visibilities, Visibility struct_visibility) {
    ...
    si->name = strdup(name);
    si->visibility = struct_visibility;
    ...
    for (size_t i = 0; i < n_fields; i++) {
        ...
        si->fields[i].visibility = field_visibilities[i];
        offset += fsize;
    }
...
int sym_register_struct_extends(SymbolTable *st, const char *name, const char *base_name,
                                 ...
                                 Visibility *field_visibilities, Visibility struct_visibility) {
    ...
    memset(si, 0, sizeof(*si));
    si->name = strdup(name);
    si->base_name = strdup_safe(base_name);
    si->visibility = struct_visibility;
    if (!si->name) return -1;
    ...
    for (size_t j = 0; j < base_si->n_fields; j++) {
        ...
        f->visibility = base_si->fields[j].visibility;
        offset += f->size;
    }
    for (size_t i = 0; i < n_fields; i++) {
        ...
        size_t fsize = get_field_size(st, field_types[i]);
        f->size = fsize;
        f->visibility = field_visibilities[i];
        offset += fsize;
    }
    si->total_size = offset;
    st->n_structs++;
...
int sym_get_struct_field(SymbolTable *st, const char *struct_name, const char *field_name, size_t *out_offset, const char **out_type, size_t *out_size, Visibility *out_visibility) {
    ...
    for (size_t i = 0; i < st->n_structs; i++) {
        ...
        for (size_t j = 0; j < si->n_fields; j++) {
            if (strcmp(si->fields[j].name, field_name) == 0) {
                ...
                if (out_visibility) *out_visibility = si->fields[j].visibility;
                return 1;
            }
        }
        return 0;
...
const char *sym_get_struct_base(SymbolTable *st, const char *struct_name) {
    if (!struct_name) return NULL;
    for (size_t i = 0; i < st->n_structs; i++) {
        if (strcmp(st->structs[i].name, struct_name) == 0)
            return st->structs[i].base_name;
    }
    return NULL;
}
